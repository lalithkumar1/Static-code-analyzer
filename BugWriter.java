	...
	public void writeData(ArrayList<T> buglist, String Classname, String AnalysisType) throws Exception {
		try {
			...
			if (((ArrayList<?>) buglist).get(0) instanceof FindBugsData)
				for (T f : buglist)
					data.put(i++, new Object[] { ((FindBugsData) f).getClassName(), ((FindBugsData) f).getCategory(),
							((FindBugsData) f).getType() });

			else if (((ArrayList<?>) buglist).get(0) instanceof PMDData)
				for (T f : buglist)
					data.put(i++, new Object[] { ((PMDData) f).getClassName(), ((PMDData) f).getData(),
							((PMDData) f).getIssue() });
			...
			FileOutputStream out = new FileOutputStream(new File(path + Classname + "-" + AnalysisType + ".xlsx"));
			workbook.write(out);
			out.close();
